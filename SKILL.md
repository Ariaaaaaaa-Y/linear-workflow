...
- 边界：这个 issue 明确不包含什么。
- 验收标准：怎样判断完成。
- 术语：当工作区使用正式术语表时，列出本 issue 使用或提出的术语 ID。
...
- 每个适用 issue 都应有 `术语` 区块；如果未确认新术语，只在 issue 的 `待解决问题`、`待确认问题` 或同义章节中记录，不写入正式术语表。
...
17. 如果决策影响正式术语表，按 `references/terminology.md` 判断是否可以写入；未确认术语只保留在 issue 的待解决问题或待确认问题中。
18. 如果决策影响长期产品文档，编辑文档前先询问是否同步。
19. 如果开发执行需要 GitHub issue 或 PR 链接，创建或更新对应 artifact，并反向链接到 Linear。
