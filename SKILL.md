...
创建或更新 issue 正文前，必须主动检查执行方是否能按正文判断“什么时候触发、什么时候完成、什么时候不该继续”。如果正文里出现“足够完整”“稳定”“清楚”“合适时机”“自然结束”“比较充分”“合理”“优先”等判断词，必须确认后面有可执行口径；没有口径时，不能把待确认问题写成“无”。
...
如果上述任何一项会影响执行或验收但没有收口，应写入 `待确认问题`，并使用工作区的待确认机制；如果工作区有 backlog / discovery 状态，应先放入对应状态。只有这些口径检查后确实不影响当前 issue，才可以写“待确认问题：无”。
...
- 边界：这个 issue 明确不包含什么。
- 验收标准：怎样判断完成。
- 术语：仅当本 issue 引入、修改、废弃、强制使用或容易混淆某些术语时才写；如果只是沿用已确认术语、没有新术语或别名，不要单独写术语区块。
...
- 只有当术语会影响执行、验收、命名或需要产品确认时，issue 才写 `术语` 区块；如果只是沿用已确认术语、没有新术语或别名，不要为了填栏目写空的术语说明。
- issue 的 `涉及术语` 中，已确认术语应写成指向正式术语表对应术语块的链接；待确认术语保持纯文本，不链接到正式术语表，避免误导为已确认收录。
...
18. 如果决策影响正式术语表，按 `references/terminology.md` 判断是否可以写入；未确认术语只保留在 issue 的待解决问题或待确认问题中。
19. 如果决策影响长期产品文档，编辑文档前先询问是否同步。
20. 如果开发执行需要 GitHub issue 或 PR 链接，创建或更新对应 artifact，并反向链接到 Linear。
...
- 不要把未确认术语写入正式术语表；确认前只在 issue 的 `术语` 和 `待解决问题` 或 `待确认问题` 中表达。
...
- 当工作区使用正式术语表时，issue 的术语区块清楚列出已确认术语和待确认术语。
